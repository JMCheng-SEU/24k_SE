...
def init_constants(Fs = 8000, Pow_factor = 0, apply_SLL_equalization=True,
    ...
    if Fs==16000:
        ...
        # Matrices
        mat = scipy.io.loadmat('D:\\real_time_exp\speech_enhancement_loss\\utils\\bark_matrix_16k.mat') # Not nice with absolute path
...
def test_PMSQE(reference_file, distorted_file):
    ...
    # tfref = tf.convert_to_tensor(reference)
    # tfdeg = tf.convert_to_tensor(degraded)
    # stft_true = K.square(K.abs(tf.contrib.signal.stft(tfref, 256, 128, 256, pad_end=True)))
    # stft_pred = K.square(K.abs(tf.contrib.signal.stft(tfdeg, 256, 128, 256, pad_end=True)))

    # PMSQE computation
    PMSQE_per_frame = per_frame_PMSQE(ref_spectra, deg_spectra, alpha=0.1)
    # PMSQE_per_frame = per_frame_PMSQE(stft_true, stft_pred, alpha=0.1)

    with tf.Session() as sess:
        pmsqe = PMSQE_per_frame.eval({signals: [reference, degraded]})
        ref_spectra_test = sess.run(ref_spectra, feed_dict = {signals: [reference, degraded]})
    print(ref_spectra_test.shape)
...
if __name__ == "__main__":
    ...
    result = test_PMSQE('D:\\real_time_exp\speech_enhancement_loss\\p226_010.wav', 'D:\\real_time_exp\speech_enhancement_loss\\p226_010.FactoryFloor1.enh.wav')
    print(result)
