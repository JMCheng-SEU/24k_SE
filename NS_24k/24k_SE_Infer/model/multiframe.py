...
class MultiFrameModule(nn.Module):
    ...
    def spec_unfold_real(self, spec: Tensor):
        if self.need_unfold:
            spec = self.pad(spec).unfold(-2, self.frame_size, 1)
            return spec.permute(0, 1, 5, 2, 3, 4)
            # return as_windowed(self.pad(spec), self.frame_size, 1, dim=-3)
        return spec.unsqueeze(-1)

    def self_spec_unfold_real(self, spec: Tensor):
        if self.need_unfold:
            spec_clone = spec.clone()

            pad_in = self.pad(spec)
            mid_out = pad_in.unfold(-2, self.frame_size, 1)

            pad_in_new = self.pad(spec_clone)

            for index in range(spec_clone.shape[3]):
                temp_win = pad_in_new[:, :, :, index:index+self.frame_size, :].permute(0,1,2,4,3)
                if index == 0:
                    spec_out = temp_win.unsqueeze(3)
                else:
                    spec_out = torch.cat((spec_out, temp_win.unsqueeze(3)), dim = 3)



            spec = self.pad(spec).unfold(-2, self.frame_size, 1)

            diff = torch.nn.functional.mse_loss(spec_out, spec)
            return spec.permute(0, 1, 5, 2, 3, 4)
            # return as_windowed(self.pad(spec), self.frame_size, 1, dim=-3)
        return spec.unsqueeze(-1)


    def spec_self_unfold_real(self, spec: Tensor):
        if self.need_unfold:
            spec = self.pad(spec).unfold(-2, self.frame_size, 1)
            return spec.permute(0, 1, 5, 2, 3, 4)
            # return as_windowed(self.pad(spec), self.frame_size, 1, dim=-3)
...
class DF(MultiFrameModule):
    ...
    def forward(self, spec: Tensor, coefs: Tensor):
        ...
        spec_real = self.spec_unfold_real(spec_clone)
