...
# Author：WangTianRui
# Date ：2021/4/4 20:29
import model.APC_SNR.acoustic_scaler as acoustic_scaler
import torch
import model.APC_SNR.pmsqe.pmsqe as pmsqe
import model.APC_SNR.conv_stft as conv_stft
...
class APC_SNR_multi_filter(torch.nn.Module):
    ...
    def __init__(self, model_hop, model_winlen, theta, mag_bins, hops=()):
        ...
        self.multi_hop = hops
        self.scaler = acoustic_scaler.AcousticScaler(theta=theta, mag_bins=(mag_bins + 1), no_grad=True)
        self.eps = 1e-8
        self.pmsqe = pmsqe.SingleSrcPMSQE(sample_rate=16000)
        self.model_stft = conv_stft.ConvSTFT(model_winlen, model_hop, mag_bins * 2, "hanning sqrt", 'complex')
        self.model_istft = conv_stft.ConviSTFT(model_winlen, model_hop, mag_bins * 2, "hanning sqrt", 'complex')
        if model_hop in self.multi_hop:
            self.multi_hop.remove(model_hop)
        self.stfts = torch.nn.ModuleList()
        for hop in self.multi_hop:
            self.stfts.append(conv_stft.ConvSTFT(model_winlen, hop, mag_bins * 2, "hanning sqrt", 'complex'))
