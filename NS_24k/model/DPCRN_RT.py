...
class DPRNN_Block_RT(nn.Module):
    ...
    def __init__(
        ...
        self.intra_rnn = nn.GRU(input_size=self.numUnits, hidden_size=self.numUnits, num_layers=1, batch_first=True)
        self.intra_fc = nn.Linear(self.numUnits, self.numUnits)

        # self.intra_ln = nn.LayerNorm(normalized_shape=[self.width, self.numUnits])

        # self.intra_ln = nn.LayerNorm(normalized_shape=[self.width, self.numUnits])
        self.intra_ln = nn.InstanceNorm2d(self.numUnits)
        ...
        # self.inter_ln = nn.LayerNorm(normalized_shape=[self.width, self.numUnits])
        self.inter_ln = nn.InstanceNorm2d(self.numUnits)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        intra_ln_input = intra_ln_input.permute(0, 3, 1, 2).contiguous()

        intra_ln_out = self.intra_ln(intra_ln_input)

        intra_out = intra_ln_out + input
        ...
        inter_ln_input = inter_ln_input.permute(0,3,2,1).contiguous()
        inter_ln_out = self.inter_ln(inter_ln_input)

        output = inter_ln_out + intra_out
...
class DPCRN_Model_RT(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.DPRNN_1 = DPRNN_Block_RT(numUnits=64, width=32)
        self.DPRNN_2 = DPRNN_Block_RT(numUnits=64, width=32)
    ...
    def forward(
        ...
        convT4_input = torch.cat((conv_out2, convT3_out[:, :,:,:-1]), 1)
