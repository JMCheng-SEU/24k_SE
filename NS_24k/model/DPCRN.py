...
class DPRNN_Block_GRU(nn.Module):
    ...
    def __init__(
        ...
        self.intra_rnn = nn.GRU(input_size=self.numUnits, hidden_size=self.numUnits // 2, num_layers=1, batch_first=True, bidirectional=True)
        self.intra_fc = nn.Linear(self.numUnits, self.numUnits)

        self.intra_ln = nn.LayerNorm(normalized_shape=[self.width, self.numUnits])

        self.inter_rnn = nn.GRU(input_size=self.numUnits, hidden_size= self.numUnits, num_layers=1, batch_first=True)
        self.inter_fc = nn.Linear(self.numUnits, self.numUnits)
...
class DPCRN_Model_512(nn.Module):
    def __init__(self):
        super().__init__()
...
class DPCRN_Model_512_Slim(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.DPRNN_1 = DPRNN_Block_GRU(numUnits=64, width=64)
        self.DPRNN_2 = DPRNN_Block_GRU(numUnits=64, width=64)


        self.convT1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=(1, 3), stride=(1, 1), padding=(0, 0)),
            nn.BatchNorm2d(64),
            nn.PReLU(),
        )
...
class DPCRN_Model_512_Slim_extra(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.DPRNN_1 = DPRNN_Block_GRU(numUnits=32, width=64)
        self.DPRNN_2 = DPRNN_Block_GRU(numUnits=32, width=64)
    ...
    def forward(
        ...
        DPRNN_out1 = self.DPRNN_1(conv_out3)
        DPRNN_out2 = self.DPRNN_2(DPRNN_out1)


        convT3_input = torch.cat((conv_out3, DPRNN_out2), 1)
        convT3_out = self.convT3(convT3_input)
