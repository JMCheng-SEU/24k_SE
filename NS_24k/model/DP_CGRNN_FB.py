...
class DPCGRNN_Block(nn.Module):
    ...
    def __init__(
        ...
        self.intra_rnn = nn.LSTM(input_size=self.numUnits, hidden_size=self.numUnits, num_layers=1, batch_first=True)

        self.intra_bn = nn.BatchNorm2d(self.numUnits)
        ...
        self.inter_bn = nn.BatchNorm2d(self.numUnits)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        intra_bn_input = intra_RNN_output.reshape(intra_RNN_input.size()[0], intra_RNN_input.size()[1], intra_RNN_input.size()[2], intra_RNN_input.size()[3])
        intra_out = intra_bn_input.permute(0, 3, 1, 2)
        # intra_out = self.intra_bn(intra_bn_input)

        intra_out = intra_out + input

        # Inter-Chunk Processing

        inter_RNN_input = intra_out.permute(0, 2, 1, 3)
        inter_RNN_input = inter_RNN_input.reshape(inter_RNN_input.size()[0], inter_RNN_input.size()[1], -1)

        ggru_1_out = self.GGRU_1(inter_RNN_input)
        inter_RNN_output = ggru_1_out.reshape(inter_RNN_input.size()[0], inter_RNN_input.size()[1], 64, -1)
        inter_out = inter_RNN_output.permute(0, 2, 1, 3)

        # inter_out = self.inter_bn(inter_RNN_output)
        output = inter_out + intra_out

        return output


class DP_CGRNN_FB_Lite_noSC(nn.Module):
    """
    ...
    def __init__(self):
        super(DP_CGRNN_FB_Lite_noSC, self).__init__()
