...
class CGRNN_FB_Lite_noSC_new_Mel(nn.Module):
    """
    ...
    def __init__(self):
        super(CGRNN_FB_Lite_noSC_new_Mel, self).__init__()

        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(1, 3), stride=(1, 1), padding=(0, 1)),
            nn.BatchNorm2d(8),
    ...
    def forward(self, x, mel_input):
        # conv
        # (B, in_c, T, F)


        x1 = self.conv1(mel_input)
        x2 = self.conv2(x1)
        x3 = self.conv3(x2)
        x4 = self.conv4(x3)
        x5 = self.conv5(x4)
...
if __name__ == '__main__':
    inputs = torch.randn(16, 1, 100, 129)

    spec = torch.randn(16, 2, 100, 257)

    Model = CGRNN_FB_Lite_noSC_new_Mel()

    enh_real, enh_imag = Model(spec, inputs)
