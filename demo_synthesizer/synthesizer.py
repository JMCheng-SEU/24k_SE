...
from demo_synthesizer.distorter import Distorter
...
if __name__ == "__main__":
    ...
    """
    s = Synthesizer("D:\\JMCheng\\RT_16k\\codes\\CGRNN_FB_512_16k\\demo_synthesizer\\synthesizer_config_16.yaml")
    out_dir = "D:\\JMCheng\\RT_16k\\DATASET\\Ant_NS_DATASET\\"

    total_hours = 150
    duration = 6

    wav_num = int(total_hours * 60 * 60 / duration)

    for index in range(wav_num):
        cur_audio = s.generate()
        nearend_wav_name = "audio%d"%index + ".wav"
        target_wav_name = "audio%d"%index + ".wav"

        nearend_path = out_dir + "noisy\\" + nearend_wav_name
        target_path = out_dir + "clean\\" + target_wav_name

        sf.write(target_path, cur_audio["target"], 16000)
        sf.write(nearend_path, cur_audio["nearend"], 16000)
        print("audio%d generate completed !!"%index)